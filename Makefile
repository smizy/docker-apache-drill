...
test:
	...
	docker-compose ps
	docker run --net vnet --volumes-from drillbit-1 smizy/apache-drill:${VERSION}-alpine  bash -c 'for i in $$(seq 200); do nc -z drillbit-1.vnet 8047 && echo test starting && break; echo -n .; sleep 1; [ $$i -ge 200 ] && echo timeout && exit 124 ; done'
